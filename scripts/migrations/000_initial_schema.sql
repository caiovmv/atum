-- 000: Schema inicial completo (estado consolidado de todas as tabelas base).
-- Idempotente: usa IF NOT EXISTS / ADD COLUMN IF NOT EXISTS em todos os statements.
...
CREATE TABLE IF NOT EXISTS downloads (
    ...
    eta_seconds REAL,
    content_path TEXT,
    content_type TEXT CHECK (content_type IS NULL OR content_type IN ('music', 'movies', 'tv', 'concerts')),
...
CREATE TABLE IF NOT EXISTS library_imports (
    ...
    content_type TEXT NOT NULL DEFAULT 'music' CHECK (content_type IN ('music', 'movies', 'tv', 'concerts')),
    name TEXT NOT NULL,
    year INTEGER,
    metadata_json JSONB,
    cover_path_small TEXT,
